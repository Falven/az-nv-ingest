...
async def list_models() -> JSONResponse:
    """List available models for compatibility with upstream clients."""
    return JSONResponse(
        {"object": "list", "data": [{"id": settings.model_name}]}, status_code=200
    )
...
async def infer(request_body: InferRequest) -> JSONResponse:
    ...
    try:
        images = await encode_request_images(request_body.input)
