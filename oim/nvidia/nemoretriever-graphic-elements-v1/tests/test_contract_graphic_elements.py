...
MODEL_ID = "nemoretriever-graphic-elements-v1"
...
def service(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Dict[str, Any]]:
    ...
    cmd = [
        ...
        "-m",
        "uvicorn",
        "oim_nemoretriever_graphic_elements_v1.server:app",
        "--host",
        "127.0.0.1",
        "--port",
        str(http_port),
...
def test_health_and_metadata(service: Dict[str, Any]) -> None:
    ...
    status, live_payload = http_json(
        "GET", f"{service['base_url']}/v1/health/live", None, headers=headers
    )
    assert status == 200
    assert live_payload == {"live": True}

    status, ready_payload = http_json(
        "GET", f"{service['base_url']}/v1/health/ready", None, headers=headers
    )
    assert status == 200
    assert ready_payload == {"ready": True}

    status, metadata_payload = http_json(
        "GET", f"{service['base_url']}/v1/metadata", None, headers=headers
    )
    assert status == 200
...
def test_infer_contract(service: Dict[str, Any]) -> None:
    ...
    populated_labels = [label for label, items in boxes.items() if items]
    assert populated_labels, "expected at least one populated label"
    sample_box = boxes.get(populated_labels[0])[0]
    assert {"x_min", "y_min", "x_max", "y_max", "confidence"} <= set(sample_box.keys())
    assert 0.0 <= sample_box["confidence"] <= 1.0
    assert 0.0 <= sample_box["x_min"] <= sample_box["x_max"] <= 1.0
    assert 0.0 <= sample_box["y_min"] <= sample_box["y_max"] <= 1.0
