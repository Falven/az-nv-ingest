...
MODEL_ID = "nemoretriever-page-elements-v3"
...
def service(tmp_path_factory: pytest.TempPathFactory) -> Iterator[Dict[str, Any]]:
    ...
    model_repo = tmp_path_factory.mktemp("model_repo")
    ...
    env.update(
        {
            ...
            "TRITON_MODEL_REPOSITORY": str(model_repo),
...
def test_health_and_metadata(service: Dict[str, Any]) -> None:
    ...
    assert info.get("shortName", "").startswith(MODEL_ID)
...
def test_infer_contract(service: Dict[str, Any]) -> None:
    ...
    populated = [label for label, items in boxes.items() if items]
    assert populated, "expected at least one populated label"
    sample_box = boxes[populated[0]][0]
