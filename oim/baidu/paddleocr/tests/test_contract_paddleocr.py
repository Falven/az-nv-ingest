...
def test_health_and_metadata(service: Dict[str, Any]) -> None:
    ...
    assert model_info[0]["shortName"] == "paddleocr"
    assert model_info[0]["id"] == MODEL_ID
    assert model_info[0]["version"]
...
def test_infer_contract(service: Dict[str, Any]) -> None:
    ...
    status, body = http_json(
        "POST", f"{service['base_url']}/v1/infer", payload, headers, timeout=10.0
    )
    assert status == 200
