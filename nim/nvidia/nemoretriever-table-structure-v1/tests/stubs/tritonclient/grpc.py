...
class InferenceServerClient:
    ...
    def get_model_repository_index(self, as_json: bool = False) -> Dict[str, Any]:
        payload: Dict[str, Any] = {"models": [{"name": "yolox_ensemble"}]}
        return payload if as_json else payload
    ...
    def infer(
        ...
        for index, _ in enumerate(values):
            ...
            predictions.append(
                {
                    "border": [[0.0, 0.0, 1.0, 1.0, 0.8]],
                    "cell": [[0.1 + offset, 0.1, 0.4, 0.4 + offset, 0.92]],
                    "row": [[0.0, 0.2, 1.0, 0.3 + offset, 0.6]],
                    "column": [[0.2, 0.0, 0.3 + offset, 1.0, 0.55]],
                    "header": [[0.05, 0.0, 0.95, 0.1 + offset, 0.5]],
                }
            )
